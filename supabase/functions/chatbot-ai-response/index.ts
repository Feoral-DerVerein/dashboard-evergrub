  ...
  try {
    ...
    if (userId) {
      ...
      if (!productsError) {
        ...
        data.expiringProducts = (productsData || [])
          .filter((product: any) => {
...
function generateDataBasedResponse(question: string, businessData: any): string {
  ...
  if (
    ...
    if (businessData.products && businessData.products.length > 0) {
      ...
      if (lowStockProducts.length > 0) {
        response += `⚠️ ${lowStockProducts.length} products have low stock (under 10 units): ${lowStockProducts
          .slice(0, 3)
          .map((p: any) => p.name)
          .join(", ")}. `;
      }
  ...
  if (
    ...
    lowerQuestion.includes("expires") ||
  ...
  if (
    lowerQuestion.includes("visitors") ||
  ...
  if (
    lowerQuestion.includes("grains") ||
    lowerQuestion.includes("grain") ||
    lowerQuestion.includes("loyalty") ||
    lowerQuestion.includes("rewards")
  ) {
    if (businessData.grains && businessData.grains.total > 0) {
  ...
  if (
    ...
    lowerQuestion.includes("who bought") ||
    lowerQuestion.includes("recent customer")
  ) {
    if (businessData.sales && businessData.sales.length > 0) {
      const recentCustomers = businessData.sales.slice(0, 5).map((s: any) => s.customer_name);
      const customerAmounts = businessData.sales
        .slice(0, 5)
        .map((s: any) => `${s.customer_name} ($${Number(s.amount).toFixed(2)})`);
