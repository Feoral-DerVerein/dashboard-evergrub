...
const MyProductsListed = ({
  ...
  const handleToggleVisibility = async (productId: number, currentVisibility: boolean) => {
    try {
      const {
        ...
        is_marketplace_visible: !currentVisibility
      }).eq('id', productId).eq('userid', user?.id);
      if (error) throw error;
      setUserProducts(prev => prev.map(product => product.id === productId ? {
        ...product,
        is_marketplace_visible: !currentVisibility
      } : product));
      toast({
  ...
  return <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {userProducts.map(product => <Card key={product.id}>
            <CardContent className="p-6">
              <div className="flex gap-4">
                <div className="w-20 h-20 bg-muted rounded-lg flex items-center justify-center">
                  {product.image ? <img src={product.image} alt={product.name} className="w-full h-full object-cover rounded-lg" /> : <Package className="w-8 h-8 text-muted-foreground" />}
                </div>
...
const Market = () => {
  ...
  const filteredOffers = marketOffers.filter(offer => offer.products.some(product => {
    const matchesSearch = product.name.toLowerCase().includes(searchQuery.toLowerCase()) || product.sku.toLowerCase().includes(searchQuery.toLowerCase()) || product.ean.includes(searchQuery);
  ...
  const renderProductCard = (product: any, offer: any) => <Card key={product.id} className="mb-4">
      <CardContent className="p-6">
        <div className="flex gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="flex flex-col justify-between">
                ...
                <div>
                  ...
                  <Button size="sm" className="w-full" onClick={() => handleReviewOffer(product, offer)}>
                    Review Offer
                  </Button>
                </div>
              </div>
  ...
  return <div className="min-h-screen bg-background">
        ...
        <Tabs defaultValue="live-offers" className="w-full">
          ...
          <TabsContent value="live-offers" className="mt-6">
            ...
            <div className="flex gap-4 mb-6">
              <div className="flex-1 relative">
                ...
                <Input placeholder="Search by Product Name, SKU, EAN, Buyer PO No., Transaction Summary No., Order Item No." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                {filteredOffers.map(offer => offer.products.map(product => renderProductCard(product, offer)))}
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="pending-delivery" className="mt-6">
            {pendingProducts.length === 0 ? <div className="text-center py-12">
                ...
                <div className="grid gap-4">
                  {pendingProducts.map(product => <Card key={`pending-${product.id}`}>
                      <CardContent className="p-6">
                        <div className="flex gap-4">
                          <div className="w-20 h-20 bg-muted rounded-lg flex items-center justify-center">
                            {product.image ? <img src={product.image} alt={product.name} className="w-full h-full object-cover rounded-lg" /> : <Package className="w-8 h-8 text-muted-foreground" />}
          ...
          <TabsContent value="products-listed" className="mt-6">
            {listedProducts.length === 0 ? <div className="text-center py-12">
                <Package className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
                <p className="text-lg font-medium mb-2">No products listed yet</p>
                <p className="text-muted-foreground">Products listed from task actions will appear here</p>
              </div> : <div className="space-y-4">
                {listedProducts.map(product => <Card key={product.listingKey || `${product.id}_${product.listedAt}`} className="p-4">
                    ...
                    setShowEditDialog(true);
                          ...
                          <Button size="sm" variant="outline" className="text-destructive hover:bg-destructive hover:text-destructive-foreground" onClick={() => handleDeleteProduct(product.id)}>
                            <Trash2 className="w-4 h-4" />
                          </Button>
                          <Badge variant="secondary" className={product.listingType === 'B2B Offer' ? "bg-purple-100 text-purple-800" : "bg-green-100 text-green-800"}>
      ...
      <Dialog open={showReviewDialog} onOpenChange={setShowReviewDialog}>
        <DialogContent className="max-w-2xl">
          ...
          {selectedProduct && selectedOffer && <div className="space-y-6">
              {/* Product Info */}
              <div className="flex gap-4">
                <img src={selectedProduct.image} alt={selectedProduct.name} className="w-24 h-24 object-cover rounded-md" />
                <div className="flex-1">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {selectedOffer.deliveryNote && <div>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={handleRejectOffer} className="bg-destructive/10 text-destructive hover:bg-destructive hover:text-destructive-foreground">
              Reject Offer
            </Button>
      ...
      {/* Payment Form */}
      {showPaymentForm && selectedOffer && <PaymentForm offer={selectedOffer} products={selectedOffer.products} onPaymentSuccess={handlePaymentSuccess} onCancel={handlePaymentCancel} />}
