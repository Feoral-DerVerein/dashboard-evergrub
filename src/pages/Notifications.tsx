  ...
  const loadNotifications = async () => {
    try {
      ...
      if (viewMode === "marketplace") {
        ...
        data = await notificationService.getAllNotifications().then(
          allNotifications => allNotifications.filter(n => !n.for_marketplace)
        );
      } else {
        ...
        <header className="px-6 pt-8 pb-6 sticky top-0 bg-white z-10">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button 
                className={`px-3 py-1 rounded-full text-sm ${viewMode === "admin" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`} 
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {filteredNotifications.map(notification => (
                <div 
                  key={notification.id} 
                  className={`flex items-start space-x-4 p-3 rounded-lg ${!notification.is_read ? 'bg-blue-50' : ''}`}
                >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      {notification.type === 'wishlist' && !notification.for_marketplace && (
                        <Badge variant="info" className="ml-2">Wishlist Item</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        {notification.type === 'wishlist' && !notification.for_marketplace && notification.product_id && (
                          <Button
                            variant="outline"
                            size="sm"
